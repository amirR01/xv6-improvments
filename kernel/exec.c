...
/**
 * Returns whether or not the file to execute starts with a shebang (`#!`).
 * Writes the interpreter's path to `interpreter`.
 */
static int
...
exec(char *path, char **argv) {
    ...
    /* Change this code.
       We must still read the header but the file might be smaller than a standard elf header and if our file contains a shebang, that is ok. */
    int file_size = readi(ip, 0, (uint64) &elf, 0, sizeof(elf));
    if (file_size < 2)
        goto bad;

    /* New code. */
    if (try_get_interpreter(&elf, interpreter_buffer)) {
        iunlockput(ip);
        end_op();
        /* We have a shebang! */
    ...
    /* If we get here, there is no shebang!
       Don't forget to check if the header size is corrrect. */
